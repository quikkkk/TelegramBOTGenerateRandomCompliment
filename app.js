...
const compliments = ['У тебя все обязательно получится!', 'Ты как столовка на Басмаче. Такая же прекрасная!', 'Ты круче всех!', 'Твой титульник самый красивый!', 'Ты горячее, чем дедлайн в 23:58!', 'Ты красивее всех опаздываешь на пары!', 'Ты как столовка на Басмаче. Такая же прекрасная!', 'Ты сегодня прекрасно выглядишь! Впрочем, как и всегда!', 'Улыбнись! Твоя улыбка прекрасна!', 'Ты сегодня молодец!', 'Улыбнись! Твоя улыбка прекрасна!', "Hey there sexy!",
 ...
 "You look awesome today, seriously", "Good morning, sunshine!",
...
bot.onText(/\/start/, msg => {
 bot.sendMessage(msg.chat.id, `*Привет ${msg.from.first_name} ❤️*, добро пожаловать в залупу! \n \n`, { parse_mode: 'Markdown' })

 setTimeout(() => {
  bot.sendMessage(
   msg.chat.id,
   `Чтобы сгенерировать комплимент, напиши /hui. ❤️`,
   { parse_mode: 'Markdown' }
  );
 }, 1000);
})
...
bot.on('message', msg => {
 ...
 if (text === '/hui') {
  ...
  const pharse = compliments[random]

  bot.sendMessage(chatId, `Лови залупу: ${pharse}`, { parse_mode: 'Markdown' })
 }
})